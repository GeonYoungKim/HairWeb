...
<%List<Map<String,Object>> QnA_search_list=(List<Map<String,Object>>)request.getAttribute("QnA_search_list"); 	
int QnAunit=(QnA_search_list.size())/10;
	...
	<div class="container">
		<div class="row">
			<div class="col-lg-8 col-md-10 mx-auto">			
					...
					<%
						if(QnA_search_list.size()>10){
							 if(QnA_page_num==0){
								for(int i=0;i<10;i++){
								%>
								<tr style="cursor:pointer;" onclick="location='/kr/QnA_one_view?QnA_no=<%=QnA_search_list.get(i).get("QnA_no")%>'">
										<TD align="center"><%=QnA_search_list.get(i).get("QnA_no")%></TD>
										<TD align="center"><%=QnA_search_list.get(i).get("QnA_preface")%></TD>
										<%if((Integer.parseInt(QnA_search_list.get(i).get("QnA_answer_flag")+""))==0){
											%>
											<TD align="center"><%=QnA_search_list.get(i).get("QnA_title")%></TD>
										<% }else{
											%>
											<TD align="center"><%=QnA_search_list.get(i).get("QnA_title")%>  <img src="resources/common/bootstrap/img/answer.png" width="50px" height="30px"></TD>
										<% }%>
										
										
										<TD align="center"><%=QnA_search_list.get(i).get("QnA_writer")%></TD>
										</tr>
								<%							
						}}else{
							for(int i=QnA_page_num-10;i<QnA_search_list.size();i++){
								%>
								<tr style="cursor:pointer;" onclick="location='/kr/QnA_one_view?QnA_no=<%=QnA_search_list.get(i).get("QnA_no")%>'">
										<TD align="center"><%=QnA_search_list.get(i).get("QnA_no")%></TD>
										<TD align="center"><%=QnA_search_list.get(i).get("QnA_preface")%></TD>
							...
							for(int i=0;i<QnA_search_list.size();i++){ %>
								<tr style="cursor:pointer;" onclick="location='/kr/QnA_one_view?QnA_no=<%=QnA_search_list.get(i).get("QnA_no")%>'">
									<TD align="center"><%=QnA_search_list.get(i).get("QnA_no")%></TD>
									<TD align="center"><%=QnA_search_list.get(i).get("QnA_preface")%></TD>
									<%if((Integer.parseInt(QnA_search_list.get(i).get("QnA_answer_flag")+""))==0){
		                ...
		                <%
		                	if(QnAunit==0){
				                ...
				                for(int i=0;i<QnAunit+1;i++){
				                	%>
				                <td><a href=/kr/QnA_search?QnA_page_num=<%=(i+1)*10%>><%=i+1 %></a></td>
				                <%}} %>
		                </tr></table>
